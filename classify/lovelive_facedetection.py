...
def detect(frame):
    ...
    for (x, y, w, h) in faces:
        ...
        print "#1 | " + MemberList[sorted_prediction_ind[0]].name + "\t" + str(predictions[0,sorted_prediction_ind[0]]*100.0)
        print "#2 | " + MemberList[sorted_prediction_ind[1]].name + "\t" + str(predictions[0,sorted_prediction_ind[1]]*100.0)
        print "#3 | " + MemberList[sorted_prediction_ind[2]].name + "\t" + str(predictions[0,sorted_prediction_ind[2]]*100.0)
        print "--------------------------------------------------------------"
        ...
        for i, value in enumerate(MemberList):
            if pred == i:
                ...
                cv2.putText(frame,value.name,(x, y), cv2.FONT_HERSHEY_SIMPLEX, int(h*0.01),(255, 255, 255),2,cv2.CV_AA)
